...
function DestinationCard({ destination, onNavigate }) {
  const shellClass = `group panel w-full border border-white/10 bg-gradient-to-br ${destination.accent} from-10% to-90% p-[1px] text-left transition hover:-translate-y-0.5 hover:border-silver/25`

  const inner = (
    <div className="flex h-full flex-col bg-slate-950/88 p-5">
      <div className="flex items-start justify-between gap-3">
        <span className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-silver transition group-hover:border-silver/30 group-hover:bg-silver/10">
          <AppIcon name={destination.icon} className="h-5 w-5" />
        </span>
        <AppIcon
          name="arrow-up-right"
          className="h-4 w-4 shrink-0 text-slate-500 transition group-hover:text-silver"
        />
      </div>
...
export function DashboardView({ checklistSections, onNavigate, planner }) {
  ...
  return (
    <div className="space-y-8">
      <section className="panel-hero min-h-[min(42vh,420px)] border border-silver/20 bg-gradient-to-br from-ucsb-navy via-[#0b2442] to-slate-950 p-6 shadow-[0_20px_90px_rgba(2,8,23,0.35)] sm:p-8">
        <div className="flex h-full flex-col justify-between gap-8">
          <div className="flex flex-col gap-8 xl:flex-row xl:items-start xl:justify-between">
            <div className="flex min-w-0 flex-1 flex-col gap-5 sm:flex-row sm:items-start">
              <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-200 to-slate-50 text-2xl font-black text-ucsb-navy shadow-[0_0_30px_var(--silver-glow)]">
                {studentProfile.initials}
              </div>
              <div className="min-w-0">
                <p className="text-label-caps">Your profile</p>
            ...
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center xl:max-w-xl">
              ...
              <div className="min-w-0">
                <p className="text-label-caps">Degree progress</p>
      ...
      <section>
        <p className="text-label-caps">Get started</p>
        <h2 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">
          What do you need help with?
        </h2>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
          Choose an area below—the same sections in the sidebar. Plan your four years, check GEs and major
          requirements, browse resources, ask questions, or review Winter deadlines.
        </p>
      ...
      {upcoming.length > 0 && (
        <section className="panel border border-white/10 bg-white/6 p-5 backdrop-blur-xl sm:p-6">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              <p className="text-label-caps">Coming up</p>
              <h3 className="mt-1 text-lg font-semibold tracking-tight">Winter 2026 deadlines</h3>
            </div>
            <button
              type="button"
              onClick={() => onNavigate('dates')}
              className="text-sm font-medium text-silver hover:text-silver-bright"
            >
              All dates & links →
            </button>
          </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-white/10 bg-slate-950/40 px-5 py-4">
        <p className="text-sm text-slate-400">
          SILVER is a planning companion—enroll and verify your record in Gaucho GOLD.
        </p>
